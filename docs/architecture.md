...
keywords: [almagest, architecture, codemap, registry, inventory, CLI]
kind: reference
---
...
`almagest` 是本机 Agent 配置控制面。当前已实现 portable Registry、Mac-local overlay、host-local binding 校验、当前主机只读 Inventory、四个 consumer adapter 和旧 skill manifest 投影；resolve、plan/apply 与 drift 仍由后续模块补齐。
...
| Inventory | `src/almagest/inventory.py` | 通过当前 host 的显式 binding 做有界只读盘点，分离期望与 live claim，生成稳定、脱敏的 snapshot。 |
| consumer adapter | `src/almagest/adapters/` | 固定 Mac Codex/QoderCLI、Windows Codex/Claude 的 root role、surface、格式 fixture、版本兼容与覆盖范围。 |
...
- **Registry 是期望权威，Inventory 是只读观测**。live 对象不能反向成为 source；没有 Apply receipt 时也不能从同名、路径或内容猜测为 Almagest-managed。详见 [[inventory-contract]]。
- **未知不能折叠成不存在**。presence、ownership、readability、topology、effective 与 coverage 必须正交表达；只有完整覆盖下才能派生 missing。
...
1. `src/almagest/cli.py` 的 `inventory scan` 接收 Registry、当前 host bindings 和 consumer evidence。
2. `src/almagest/inventory.py:scan_inventory` 先执行 Windows local-layer zero-probe gate，再复用 Registry 校验。
3. target 的 platform/consumer 选择 `src/almagest/adapters/` 中的 adapter；evidence 决定能否做语义解析。
4. adapter 只把语义 role 映射到显式 binding root；bounded reader 拒绝路径和 symlink 逃逸。
5. Inventory 分别生成 binding、expected、live claim，再计算 coverage、duplicate/effective 与稳定 snapshot revision。
6. CLI 只输出 root-relative action context 和 allowlist 状态；`partial`、`block` 都返回非零。
...
| 改 live claim、coverage 或安全输出 | `src/almagest/inventory.py` | 同步 `docs/inventory-contract.md` 和 `tests/test_inventory.py`。 |
| 改 consumer root、格式或 precedence | `src/almagest/adapters/` | 必须更新 adapter revision、兼容 fixture、契约和实机证据。 |
...
| 接入 resolve / plan / apply | 新模块 | 不要塞进 Registry loader 或 adapter；分别由后续 ERI-948/949/950 实现。 |
...
- Registry 本身不发现 live 配置；Inventory 不渲染 consumer 格式、不写目标文件，也不验证声明 revision 已在本机物化。
