...
- **host-local binding**：当前 host 为 path、account 或 secret reference 提供的本机值。binding declaration 通过稳定 `role` 说明用途；binding 只补值，不取得 authored authority。
...
| RG-9 | MUST | host-local binding 只能使用 `path`、`account` 或 `secret-ref`；declaration 必须提供稳定语义 `role`；secret 只能登记 provider/reference，不得内联 value。 | discriminated schema、role 与 secret 负向测试。 |
...
| RG-14 | MUST | 当前 host 上每个 target 的 target-level binding 都必须校验，即使该 target 没有 assignment；同一 target 不得声明两个相同语义 role。 | zero-assignment target 与 duplicate-role 测试。 |
...
|---|---|---:|---:|
| macOS | Codex | 是 | 是 |
| macOS | QoderCLI personal target | 是 | 否 |
| macOS | QoderCLI local work target | 否 | 是 |
| Windows | Codex | 是 | 否 |
| Windows | Claude | 是 | 否 |
...
- Inventory 通过 `BindingDeclaration.role` 消费 root，不根据 binding ID、路径或 consumer 默认 home 猜测用途。详见 `docs/inventory-contract.md`。

## 不属于本阶段

Registry 不做 live inventory、consumer capability probe、render、dry-run plan、apply/rollback、drift 或 source revision materialization 校验。ERI-946 Inventory 已消费本 Registry；后续能力也不能另建第二套配置 authority。
