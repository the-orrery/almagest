...
状态：生效。适用范围：`almagest` 读取的所有 `skills.json` manifest、其 source-root overlay，以及 `doctor --json` 消费方。变更须经维护者审阅，并同步实现、测试与 manifest 示例。
...
路径型 source 的存量兼容是唯一默认豁免；不得为新 capability 新增绝对路径。任何新增 source 解析模式、JSON 字段或失败代码，必须先更新本契约与 CLI 测试，再合入实现。
