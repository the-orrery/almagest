...
```gherkin
Scenario: 用 per-scope coverage map 显式表达完整、部分与不可访问范围
  Given 当前调用已固定本机 host、declared targets、adapter/capability manifest 与 boundary revision
  When Almagest 开始生成 inventory snapshot
  Then 必须先生成声明 coverage units 的 snapshot boundary
  And 每个 declared scope 必须产生 complete、partial、unavailable 或 not-applicable coverage record
  And coverage record 必须包含 coverage ID、target、stage/fact domain、opaque scope ID/type、adapter/boundary revision、state、reason 与 attempt/evidence references
  When adapter 按固定 complete condition 枚举完 declared scope 的全部 segment
  Then coverage state 为 complete
  And complete 只表示枚举完整，不得提升 scope 内任一 claim 的 evidence level、ownership、drift 或 effective state
  When declared scope 只枚举了一部分
  Then coverage state 为 partial
  And 必须用稳定 segment references 列出 covered/uncovered 区域与原因
  And 已取得 claim 保留各自 07C evidence，不得外推未覆盖区域
  When declared scope 因权限拒绝、adapter unsupported、必要边界未配置或版本不兼容而完全无法枚举
  Then coverage state 为 unavailable
  And scope 下当前 claim 为 unknown/typed failure
  And 不得报告 empty、absent 或 external-owned
  When 固定 target capability、eligibility 或 policy rule 明确证明一个 declared scope 不适用
  Then coverage state 可以为 not-applicable
  And 必须引用该正向 rule/policy evidence
  When 没有正向 not-applicable evidence
  Then 不得用 not-applicable 隐藏权限、adapter 或 boundary 缺口
  When 已知 root 被直接确认不存在
  And 证明 absence 所需的所有 coverage units 都 complete
  Then 可以记录 complete coverage 与 07C observed absence
  When root 未知、权限拒绝、遍历 partial 或 adapter failure
  Then 不得产生 presence=absent、唯一 winner 或 collision-free 结论
  When 当前调用发生在 Mac
  Then Windows host/targets 不进入 snapshot boundary
  And 不得标成 unavailable 或生成跨机 coverage
  When 当前调用发生在 Windows
  Then Mac host 与 Mac-local work scope 不进入 snapshot boundary
  And 不得以 not-applicable 条目泄漏 work 的存在、名称、数量或元数据
  When action 依赖 partial/unavailable scope 中的 absence、winner、collision-free 或其它完整性前提
  Then 该写动作必须阻断并返回 coverage ID 与安全 resolution actions
  And 不依赖该 scope 的 inventory/explain 与独立 target 可以继续
  When principal 要修复 coverage 缺口
  Then Almagest 不得自动扩目录、提权、读取 fallback/cache 或访问另一台 host
  And 受管 boundary/adapter 修改必须形成新 revision/plan，外部权限由其 owner/tool 修复
  And 修复后必须生成新的 immutable snapshot，不得改写旧 coverage record
  When 返回 coverage 摘要或详情
  Then 默认摘要只包含状态计数、阻断 scope IDs 与 coverage detail ID
  And 详情必须遵守 DEC-04D/06C，不得泄漏敏感路径、secret 或 work 内容/元数据
```
