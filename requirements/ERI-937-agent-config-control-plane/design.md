...
- 决策轴：
  ...
  - 03B2：检测到 cache、resolved、rendered 或 live 反向污染 source 后，是否自动隔离/恢复，以及如何阻断、取证和取得 principal 决定。污染证据类型、证据等级和 freshness 合同由 DEC-07/16 细化。
...
| `observation` | target/consumer/schema/capability 等解析与验证实际使用的 host fact identity、status 与 evidence reference | 不把 observation 当 desired value；其 observed/inferred/unknown 与 freshness 按 07C 类型化证据信封表达 |
...
- Snapshot 与 authored inventory 边界：`stage inventory snapshot` 是某次本地调用的只读结果，不写回 05D source inventory、payload 或 binding registry。snapshot identity、采集边界和固定输入必须可引用；07C 已固定 observed/inferred/unknown、采集元数据与 freshness 语义，plan 如何冻结 snapshot 由 09B 决定。
...
- Bounded live discovery：live 只读取 target/adapter 声明的配置 roots、文件类型、config keys 与引用边界；07D 已要求每个声明 scope 产生 coverage record，并将边界外对象保持为 out-of-scope。具体多 root、precedence、shadow 语义由 08A 决定。边界外 host 文件、process、package、cache、history、session 与模型数据不因本轴进入全盘扫描。
...
- 后果：07B 已固定各 stage 的 control/integrity 分类且不改变 identity；07C 已为 observation/evidence 固定证据等级、采集元数据与 claim-specific freshness；07D 已用 per-scope coverage map 表达 bounded discovery、权限和 partial snapshot；DEC-08 固定 rendered/live mapping；DEC-09B 固定被批准 plan 使用的 stage snapshot 与 adapter versions；DEC-12A 选择要比较的 stage edges；DEC-16 通过 record ID 串起 source→live provenance，Later 再关联 DEC-11 effective evidence。
...
- External proof：`external-owned` 必须有可引用的外部 owner identity 与 ownership evidence。Almagest source/provenance 缺失只能得出“非已证 managed”，不能直接得出 external-owned；证据不足、冲突或过期时降为 `unknown-owner`。证据等级、采集元数据与 freshness 按 07C 的类型化证据信封判断。
...
#### 07C 已拍板：类型化证据信封与 claim-specific freshness

本轴决定 inventory 中的每项事实如何说明“这是直接看到的、确定性推导的，还是当前不知道”，以及证据何时仍可用于当前判断。证据等级描述取得事实的方法，不授予 source authority、写权限或更高 effective 等级。

| 选项 | 证据模型 | 结论 |
|---|---|---|
| A：Direct-only | 只有直接读取才有事实，其余全部 unknown | 拒绝：虽然简单，但会丢掉由固定 source、binding、schema 和 adapter 可复算得到的 resolved/rendered 等确定性事实 |
| B：类型化 evidence envelope | observed、inferred、unknown 三态；保存方法、版本、时间、来源引用和按 claim 定义的 freshness | **已选择** |
| C：置信度分数 | 为每项事实计算 0—1 或高/中/低分数，由阈值决定是否可信 | 拒绝：分数来源与阈值难以复现，容易把启发式或 Agent 判断伪装成确定性证据 |
| D：Latest-known wins | 最近一次成功证据持续有效，直到新证据覆盖 | 拒绝：会把 consumer、权限、路径或 live 已变化后的陈旧结果继续当成当前事实 |

```text
claim
└── evidence envelope
    ├── level: observed | inferred | unknown
    ├── subject + claim kind
    ├── method + collector/adapter/schema/rule versions
    ├── snapshot + collected/evaluated time
    ├── source/input evidence refs
    ├── freshness policy + current evaluation
    └── unknown/conflict/stale diagnostic
```

| Evidence level | 成立条件 | 不得声称 |
|---|---|---|
| `observed` | 当前 snapshot 内，由声明的 collector/adapter 在获准边界直接读取目标，并按固定 schema 解析或验证；`present`、`absent`、具体状态与读取失败必须分开表达 | 不因文件存在或可解析而升级为 consumer loaded/enabled/callable；不把越界猜测写成 direct read |
| `inferred` | 仅由已引用且当前有效的输入证据，通过命名、版本化、确定性的 derivation rule 得出；相同输入与版本必须产生相同结论 | 不提升为 observed；不允许概率、相似度、路径惯例、LLM 判断或未登记 fallback 冒充确定性推导 |
| `unknown` | 未采集、方法不支持、权限/读取失败、证据不足、输入冲突、版本不兼容或 freshness 无法证明；必须附稳定 reason 与已尝试 evidence reference | 不等于 absent、false、external-owned、未配置或失败成功；不得用旧值、默认值或 latest-known 静默补齐 |

- 决定（v0.1，2026-07-17，approver: principal）：07C 选择 **B——类型化 evidence envelope**。每个 inventory claim 必须通过 evidence ID 引用一个不可变证据信封；默认摘要只返回 level、freshness、reason 与 evidence ID，完整方法和 provenance 按 ID 获取，避免把整条证据链重复塞入 Agent 上下文。
- 最小信封：至少包含 subject/claim kind、`observed|inferred|unknown`、method ID/kind、collector/adapter ID 与版本、相关 schema/rule/policy 版本、snapshot ID、采集/求值时间，以及安全的 source/input evidence references。consumer version 影响解释时必须记录；无法取得时本身是 unknown，不使用空字符串冒充已知。
- Direct observation：observed 只描述“在什么边界用什么方法直接取得了什么结果”。直接确认不存在可以是 observed absence；权限拒绝、root 未知、read/parse 失败只能是 unknown/typed failure，不能写成 observed absence。
- Deterministic inference：inferred 必须引用全部决定性输入 evidence IDs 和 versioned derivation rule。任一输入 unknown、冲突、失效或版本不兼容时，依赖 claim 也变为 unknown；Almagest 不输出任意 confidence score，也不让 operator Agent/LLM 以解释文字补足推导链。
- Freshness 按 claim 定义：不可变、revision-bound 事实在所引用 revision、schema/rule 和输入未变时可以持续有效；权限、登录、consumer version、active root、文件/live 状态等可变事实必须声明 `current snapshot` 或 adapter/policy 给出的最大年龄/失效条件。禁止对所有 evidence 使用一个全局 TTL，也不跨 Mac/Windows 比较时钟。
- 失效语义：证据信封保持不可变；当时间、输入 revision、adapter/schema/consumer compatibility 或 policy 使旧证据不再满足 freshness 时，旧 envelope 作为 stale history 保留，新一轮当前 claim 返回 unknown 并引用 stale evidence 与原因。互相冲突的 envelopes 全部保留，当前 claim 为 unknown，不按最新时间、最高版本或任意优先级自动选 winner。
- 使用门禁：DEC-09/10 的 action policy 后续声明每类动作需要的最低 evidence level 与 freshness。inferred 不能满足显式要求 observed 的前置条件；unknown 必须阻断依赖该 claim 的写动作，但不阻断安全的 inventory/explain 与不依赖它的 target。principal approval 可以选择取新证、修 source/binding，或批准 policy 明确允许的降级计划，但不能把 evidence label 直接改成 observed/inferred。
- Authority 与安全边界：observed/inferred 都不会因此取得 authored authority、managed ownership 或 mutation authority。证据信封和按 ID 详情继续遵守 04D/06C；不得内联 secret、provider locator、Mac-local work 内容/元数据、原始 payload 或 raw error，只保存允许披露的 identity、digest、版本、状态与引用。
- `Must`：observed/inferred/unknown 三态；确定性 inference；不可变 typed envelope；method/version/time/provenance；claim-specific freshness；冲突/过期降为当前 unknown；unknown 有稳定 reason；默认紧凑、按 ID explain；classification/authority 与 evidence level 分离。
- `Later`：DEC-11 effective probe 复用同一 envelope primitive，但使用独立 evidence namespace 和各 effective level 的 method/freshness contract；新增 collector 方法时以 adapter capability + fixture 扩展。
- `Out`：主观 confidence score、LLM/人工猜测作为 inference、全局统一 TTL、latest-known 自动沿用、冲突自动择新/择高版本、手工改 evidence label、把 inferred 包装为 observed、把 unknown 当 absent/default，以及为证据留存复制原始敏感内容或生成跨机时间线。
- 接受的代价：adapter/schema 必须逐 claim 声明方法、版本和 freshness，inferred claim 还要保存输入引用；权限或短暂读取故障会产生更多 unknown/block。以此换取 Agent 能区分“直接看到”“可复算推导”“现在不知道”，且旧证据不会在环境变化后静默继续驱动写入。
- 后果：07D 已固定 coverage/partial snapshot 且不重定义单条 claim 的 unknown；09B 固定 snapshot、evidence IDs 与 adapter/schema/rule versions，09D 在输入或 freshness 变化后使旧 plan/approval 失效；10A/10B 定义各动作最低证据门槛；11B—11D 复用 evidence level 但不能让 runtime evidence 覆盖 v1 stage truth；12 重评 freshness 并区分 stale evidence 与真实 drift；16 通过 evidence ID 提供紧凑摘要与按需 explain。
- 验收断言：直接读取与确定性推导不会混标；推导可由输入 IDs 和 rule version 重放；权限失败不冒充 absent；冲突、过期或版本不兼容时当前 claim 为 unknown 且旧证据仍可审计；没有统一 TTL 或 latest-known fallback；unknown 只阻断依赖它的动作；principal/Agent 均不能通过改标签提升证据等级；所有 evidence 输出继续满足 work 零离机和 safe-view 约束。

#### 07D 已拍板：声明边界内的 per-scope coverage map

本轴决定一次 inventory 如何说明“计划查看哪些范围、实际覆盖到哪里、哪些区域没有看完”。coverage 只描述枚举覆盖，不评价单条 claim 的证据等级；一个 scope 可以枚举完整但其中某项 claim 为 unknown，也可以 scope 仅部分覆盖但已取得的个别 observed claim 仍然有效。

| 选项 | Coverage 模型 | 结论 |
|---|---|---|
| A：全有或全无 | 任一 scope 不可读就使整份 snapshot 失败并丢弃其它结果 | 拒绝：安全但过度阻断，无法保留不受影响 target/scope 的有效证据 |
| B：Per-scope typed coverage map | 每个声明 scope 记录 complete、partial、unavailable 或 not-applicable，并列出缺口与原因 | **已选择** |
| C：Silent best effort | 返回成功发现的对象，不记录未扫描范围 | 拒绝：调用方无法区分“范围内不存在”和“根本没有看到”，会制造假 absence |
| D：自动扩扫/提权/跨机 fallback | 权限或边界不足时自动扩大目录、读取 cache、提权或访问其它 host | 拒绝：突破 bounded discovery、最小权限、work 零离机和本机即时操作边界 |

```text
snapshot boundary（current host + declared targets + adapter manifest）
└── coverage units
    ├── complete
    ├── partial
    ├── unavailable
...
other host / undeclared root / undeclared domain ──> out-of-scope，不是 unavailable
```

| Coverage state | 成立条件 | Claim 语义 |
|---|---|---|
| `complete` | 当前 snapshot 已按固定 adapter/version 枚举完该 declared scope，未跳过任何 scope-defined segment | 只证明枚举完整；具体对象仍按 07C 分别为 observed/inferred/unknown |
| `partial` | declared scope 只有一部分完成，且未覆盖 segment 可以用稳定 scope/segment ID 与原因列出，例如局部权限拒绝、遍历中断或固定边界预算耗尽 | 已取得 claim 保留自身证据等级；未覆盖 segment 中不得产生 absence 或完整性结论 |
| `unavailable` | declared scope 完全无法枚举，例如权限拒绝、adapter 不支持、必要边界未配置或版本不兼容 | scope 下当前 claim 为 unknown/typed failure；不得把 scope 标为 empty、absent 或 external-owned |
| `not-applicable` | 当前 target 的已固定 capability/policy 明确证明该 declared scope 不适用，并保存 rule/policy evidence | 不是错误，也不生成对象 absence；不得作为缺 adapter、缺权限或未配置边界的兜底状态 |

- 决定（v0.1，2026-07-17，approver: principal）：07D 选择 **B——声明边界内的 per-scope typed coverage map**。每次 snapshot 必须先固定本机 host、target 集合、adapter/capability manifest、boundary revision 与 coverage units，再执行枚举；snapshot 与 coverage records 均不可变，修复后重试产生新 snapshot。
- Coverage unit：至少包含 coverage ID、target、stage/fact domain、opaque scope ID/type、adapter 与 boundary revision、coverage state、稳定 reason code、attempt/evidence references，以及 partial 时的 covered/uncovered segment references。默认摘要只返回各状态计数、阻断 scope IDs 与 coverage detail ID；路径和其它敏感细节按 06C safe view 获取。
- Complete 不等于成功：coverage complete 只说明声明范围已枚举完，不把 parse failure、ownership unknown、drift 或 runtime effective 变成成功。反向也成立：partial/unavailable 不抹掉已取得 claim 的 07C evidence，但不能让这些 claim 外推未覆盖区域。
- Absence gate：只有证明某项不存在所需的全部 coverage units 都是 complete，且直接方法确实得到 07C observed absence 时，才能报告 `presence=absent`。已知 root 被成功确认不存在可以是 complete + observed absence；root 未知、权限拒绝、adapter failure 或部分遍历只能是 partial/unavailable + unknown。
- Not-applicable gate：`not-applicable` 必须引用固定 target capability、eligibility 或 policy rule。没有正向证据时使用 partial/unavailable/unknown，不得用 not-applicable 隐藏缺口。Windows snapshot 的 boundary 根本不包含 Mac-local work scope，也不得用 not-applicable 条目泄漏其存在、名称、数量或其它元数据。
- 本机边界：Almagest 只盘点当前调用所在 host。另一台机器、未声明 target、未声明 root/domain 不进入本次 boundary，是 out-of-scope 而不是 unavailable；不得生成远端 coverage、跨机 partial 状态或中央汇总。principal 要检查 Windows 时，由 Windows 上的 operator Agent 独立生成 Windows snapshot。
- 阻断粒度：partial/unavailable 使依赖未覆盖 scope 的 absence、winner、collision-free 或其它完整性前提变为 unknown，并阻断依赖它们的写动作；不依赖该 scope 的安全 inventory/explain 与独立 target 可以继续。09/10 后续固定 action→required coverage/evidence preconditions，coverage 本身不授予 mutation authority。
- 恢复出口：Almagest 返回 allowlisted resolution action，例如修复外部权限、补合法 adapter/boundary 配置或在正确 host 重新运行；不自动扩目录、提权、读取 fallback/cache、放宽边界或远程访问。principal 选择会修改受管配置的动作时，仍须形成新 revision/plan；外部权限修复由对应 owner/tool 完成，随后重新 inventory。
- `Must`：snapshot boundary manifest；per-scope complete/partial/unavailable/not-applicable；partial 明示缺口；absence 需要 complete coverage；coverage 与 07C evidence 正交；当前 host only；受影响动作 fail closed；默认紧凑、按 ID explain；04D/06C 安全边界。
- `Later`：超大合法 scope 的确定性 pagination/resume、性能统计和更多 adapter-specific coverage diagnostics；不得把分页中间态包装成 complete，也不得扩成后台 daemon 或跨机聚合。
- `Out`：全局单一 complete 布尔值、silent best effort、未覆盖即 absent/empty、not-applicable 兜底、全磁盘扫描、自动扩大 root、自动 sudo/提权、读取未声明 cache/history/session、远程 host discovery、跨机 coverage report，以及 partial 时丢弃所有已取得证据。
- 接受的代价：每个 adapter 要声明可枚举 scope 与完成条件，partial 还要稳定标识缺口；权限故障或大目录边界会产生更多 coverage records 和局部 block。以此换取 Agent 能精确说明“哪里看完、哪里没看完”，而不会把没权限或没扫描误报成配置不存在。
- 后果：DEC-07 四轴全部完成；08A 使用 complete coverage 才能证明 root winner/shadow 集合完整，partial/unavailable 时只能报告已知 claimant 与 unknown winner；09B 固定 boundary/coverage snapshot，09C 汇总 scope-level block，09D 在 coverage 输入变化后失效；10A/10B 保护未覆盖资产；12 区分真实 absence/drift 与 observation gap；13 保持各 host 本地独立；15 adapter manifest 声明 scopes/complete condition；16 按 coverage ID 解释缺口和恢复历史。
- 验收断言：每个 declared scope 都有 coverage record；complete 不提升单条 evidence，partial 不抹掉已取得 evidence；权限失败不产生 absence；not-applicable 有正向规则；另一台 host 是 out-of-scope；未覆盖区域不会参与 winner/collision-free 推断；相关写动作阻断而独立只读工作继续；Almagest 不自动扩扫、提权或跨机；所有 coverage 输出满足 work 零离机和 safe-view。
