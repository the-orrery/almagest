...
- secret value 生命周期继续由外部 credential provider 持有；06A 不决定 provider、存放、binding scope、脱敏或缺值恢复，分别保留给 06B—06D。分类未知时 `unknown_local_role/block`，不得按 non-secret、环境变量或 adapter 惯例猜测。
...
- value/locator/identity/observation 的披露与脱敏现由 06C 固定；06D 仍需决定零匹配、dangling locator、权限与 value validation 的诊断和恢复。
...
- 06D 仍需定义缺值、ambiguous、dangling locator、权限和 validation error 的 fail-closed 状态与 resolution action，但必须全部通过 06C safe diagnostic 表达。
...
- DEC-06D—DEC-16 的拍板结果：后续逐项写入 `design.md`。
