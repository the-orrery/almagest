...
- B/C/D 分别因未授权 source 修改、误覆盖 authored change 风险和把 mutation authority 隐藏进证据分类而拒绝。污染证据等级和 freshness 现由 DEC-07C 固定，审计解释仍由 DEC-16 承接；03B2 不授予任何 mutation authority。
...
- rendered/live/cache/session/`external-owned`/`unknown-owner` 本机状态没有 authored authority。接纳本机差异必须显式修改合法 source 并生成新 plan，不能把 live target 或任意 local 目录提升为 override。
...
- override/mask 只允许 Mac-local work 无歧义地指向 eligible base 的 semantic target。target 缺失/歧义、shape 不兼容或多个意图竞争均 conflict/block；05D 已确定 operation/target ref 进入 source inventory，并绑定 stable target locator + expected target semantic digest。
...
- 只有 inventory 登记且在当前 source root 内唯一解析的 payload 才进入 candidate set。invalid schema、duplicate ID、dangling/ambiguous/root-escaping ref 均 block；unlisted payload 不自动 adopt，只作为 `orphan + unknown-owner` evidence，除非存在正向 external owner proof。
...
- 2026-07-17，principal 对 target reference 选择 B：override/mask 同时绑定 stable logical/subresource/item locator 与精确目标单元的 expected canonical semantic digest。A/C/D 分别因静默吞掉目标更新、同资产无关变化误报和任意 base 变化引发全局 stale 而拒绝。
- target digest 由固定 adapter/schema 对精确语义单元 canonicalize 后计算，不使用 raw bytes、path、mtime、whole asset 或 source revision。目标格式/位置或无关内容变化只使旧 plan 失效；重新 plan 后 target digest 相同则意图仍有效。目标语义变化、消失或 schema identity 不可验证时 `stale_target/block`，禁止自动刷新或扩大目标。
- principal 可要求 Agent 更新/删除 override/mask 或确认新 base 后更新 expected digest，形成新 work revision 并重新 plan；单次处理仍走 03B1，不持久刷新。具体 inventory 字段名、序列化、digest algorithm 标识和物理分片属于实现细节。

## DEC-06A 拍板结果

- 2026-07-17，principal 选择 B：所有 secret、路径、账号与 host-dependent 表示按职责分成 portable declaration/reference、host-local binding value/provider locator、observed host fact；分类不按名词或 secret/non-secret 二分。
- portable declaration/reference 由既有两个 authority source 声明 typed slot、required/optional、sensitivity 与约束；host-local binding 只为当前 target 满足已有 slot；observed host fact 只作为 selector、capability、plan/verify evidence 或诊断输入。binding 与 observation 都不形成第三 authored layer。
- 同一概念按阶段分别分类：账号需求/逻辑引用是 declaration，本机选中账号/profile 是 binding，登录与权限状态是 observation；路径 slot、实际绝对路径、路径存在性/权限也按同样方式分离。
- secret value 生命周期继续由外部 credential provider 持有；provider/存放/scope、安全披露与缺值恢复现分别由 06B—06D 固定。分类未知时 `unknown_local_role/block`，不得按 non-secret、环境变量或 adapter 惯例猜测。
- A/C/D 分别因 binding 与 observation 混淆、非 secret 本机事实泄漏、以及跨 adapter authority 不一致而拒绝。接受每个 adapter/schema 显式标注 local-sensitive role、旧配置含混时先阻断补分类的成本。

## DEC-06B 拍板结果

- 2026-07-17，principal 选择 B：每台机器维护一份逻辑 typed host-local binding registry；entry 按 stable slot ID、显式 host-wide/exact-target scope 与 fill/replace-ref mode 保存非 secret 本机值或 provider locator。具体文件名、序列化与物理分片留作实现细节。
- 环境变量、Keychain/credential manager、本机文件等只有被 entry 显式引用时才是 provider；禁止扫描、按名字猜测或自动 adopt。secret value 不进 registry，生命周期仍由外部 credential provider 持有。
- source slot contract 决定 allowed scope 与是否 replaceable。每个 slot/target 至多一个候选；多个候选 `ambiguous_binding/block`，不做 scope/provider precedence。默认只 fill/resolve，replace-ref 必须显式授权，且不能覆盖 concrete portable value 或普通 authored 字段。
- registry 是 host-local managed input，不是 authored source。operator Agent 修改 registry 必须生成绑定 revision/digest 的 DEC-03D non-no-op plan并取得 principal 批准；registry、locator、存在性和 digest 不进 Git、不跨机同步或汇总。
- A/C/D 分别因隐式依赖不可盘点、live/rendered 反向取得 authority、以及重建第三 local overlay 而拒绝。接受每台机器独立维护显式 registry、冲突时不猜 precedence 的成本。
- value/locator/identity/observation 的披露与脱敏现由 06C 固定；零匹配、dangling locator、权限与 value validation 的诊断和恢复现由 06D 固定。

## DEC-06C 拍板结果

- 2026-07-17，principal 选择 B：plan、diff、receipt、日志、错误与 explain 均从 rich internal state 生成 schema-driven、per-surface safe view；安全投影发生在 serialization/logging 前，不使用 serialize-then-scrub 或字段名正则作为主防线。
- portable-safe 可在 plan/diff 或按 ID explain 中提供精确 before/after；local-sensitive 默认只显示 slot/entry ID、type、scope、change kind、状态与结构化 redaction marker；unknown sensitivity 隐藏并 `unknown_sensitivity/block`。
- secret value 永不进入 reportable object、hash、fingerprint、exception、trace、telemetry、cache key 或 reveal，只报告安全状态和 provider/registry 已有 revision evidence。provider 原始 stdout/stderr/exception/response 不透传，必须映射为 allowlisted typed diagnostic。
- non-secret local-sensitive value 仅可在当前 host 的单字段定向 explain 中，经 principal 对 current host/target/plan-or-explain/field/调用的单次确认后显示；不可批量、跨 host、持久化或写入 receipt/log/cache。work-derived redaction metadata 继续全量 Mac-only。
- A/C/D 分别因正则漏报、审阅信息不足、以及完整本地输出先进入 Agent/log/error 而拒绝。接受维护 sensitivity schema、多套 surface DTO、safe provider diagnostic 与受控 explain 的成本。
- 缺值、ambiguous、dangling locator、权限和 validation error 的 fail-closed 状态与 resolution action 现由 06D 固定，并全部通过 06C safe diagnostic 表达。

## DEC-06D 拍板结果

- 2026-07-17，principal 选择 B：本机参数解析失败采用类型化 fail-closed；Almagest 返回安全 diagnostic、影响边界与有限 resolution action，operator Agent 向 principal 解释并按其现场选择进入既有 authority/plan 流程。
- required 缺值、ambiguous、dangling locator、权限/登录失败、invalid value、required observation 不满足、provider unavailable 与未知分类均阻断受影响 target；不相关的只读 inventory/diff/explain 与健康 target 可继续，但多 target partial apply 仍留给 DEC-09/10。
- optional 只有在 portable source 明确声明 optional，且固定 adapter/schema 证明 omission deterministic 且安全时，才可成为逐项可见、随精确 plan 获批的 `optional_omitted`；否则按 required block。
- diagnostic 只含 stable ID/code、target/slot opaque identity、阶段、影响、固定 revision evidence 与 allowlisted action kind，遵守 06C；不得包含 secret、locator、本机路径、账号或 raw provider error。一次调用聚合所有不跨安全边界即可确定的独立 blocker/omission。
- resolution action 不是自动执行授权。受管 source/registry 修改走 DEC-03D 新 plan，外部 provider/权限由其 owner/tool 修复；之后必须重新读取、resolve、validate 和 plan。acknowledgment、reveal 或旧 plan/approval 不能解除阻断。
- A/C/D 分别因 optional 语义失效并让可安全省略项阻断 target、隐式候选重新取得 authority、以及陈旧/已撤销值与 secret cache 风险而拒绝。接受短暂故障也会停止受影响 target，并要求 schema 明确 optional omission 语义的成本。

## DEC-07A 拍板结果

- 2026-07-17，principal 选择 B：v1 按 logical asset/subresource 生成 `source → resolved → rendered → live` stage inventory snapshot，binding 与 observation 作为带 provenance 的 side facts 分栏；DEC-11 effective evidence 属 Later 且使用独立 evidence set。
- 05D source inventory 是 authored 控制元数据；07A stage snapshot 是本机单次只读结果，没有 source authority。共同主键语义为 snapshot/stage/target/logical asset/optional subresource，物理 path/key 仅作 stage evidence，不替代 logical identity。
- 每个 stage 保存 identity、presence/state、revision/digest 与直接上游 provenance。invalid、blocked、unresolved、missing/unreadable 都形成显式断点；rendered success 不等于 live materialized，live present/parsed 也不等于 runtime loaded/enabled/callable。
- live discovery 只进入 adapter/target 声明的 config root、file type、config key 与引用边界；边界内 unmapped candidate 可保留 opaque evidence，但不自动取得 logical/managed/source identity 或 adopt/delete 权限。07B/07C 已固定 classification 与 evidence level/freshness；07D 已固定 per-scope partial discovery。
- inventory 不复制 payload/render/live 正文，精确内容按 ID 通过 06C safe diff/explain 读取；binding/secret/local-sensitive/work 继续服从 04D/06C。effective evidence 只能关联 v1 stage record，不能覆盖或反写配置事实。
- A/C/D 分别因两端 diff 难以定位中间阶段、完整物理快照扩大敏感面并演化成 host manager、以及 runtime probe 不稳定且可能反向主导 desired config 而拒绝。接受 adapter 增加 stage mapping/digest/lineage 与显式 missing record 的成本。

## DEC-07B 拍板结果

- 2026-07-17，principal 接受推荐方案：control ownership 与 integrity 正交分类，并将 active model 的 `unmanaged` 更名为 `external-owned`。control 为 managed/external-owned/unknown-owner；integrity 内部分为 linked/orphan 与 unique/duplicate。
- managed 必须有当前合法 source/registry 或固定 plan/apply/receipt ownership evidence；external-owned 必须有正向 external owner identity/evidence；缺少 managed provenance 不能反推出 external-owned，证据不足、冲突或失效均为 unknown-owner。其证据等级与 freshness 已由 07C 的类型化证据信封固定。
- control 与 integrity 可组合：managed+orphan、external-owned+duplicate、unknown-owner+orphan+duplicate 均可表达。orphan 只表示“实际存在但无合法当前直接 upstream relationship”，不否定独立 ownership evidence；expected missing 保持 07A presence=absent。duplicate 不自动产生 winner。
- classification 不授予 mutation authority。managed 仍需 plan/approval；external-owned 只读；unknown-owner fail closed。发现 orphan/duplicate 不自动 adopt、rewrite、move、deduplicate 或 delete，也不默认阻断整台 host，但影响这些对象或与 managed desired collision/shadow 的 target action 必须阻断。
- principal 决定接纳时不能翻转 classification flag；Agent 必须先修改合法 GitHub/Mac-local source 或 host-local binding，形成新 revision/plan，重新 inventory 后由 provenance 得到 managed。Almagest 不从 live/external object 反向吸收。
- A/C/D 分别因扁平枚举丢失组合事实、source 缺失被误当 external proof、以及启发式推断/自动归并扩张 authority 而拒绝。接受每条 record 增加 control/link/cardinality/evidence 与来源不明对象持续告警的成本。

## DEC-07C 拍板结果

- 2026-07-17，principal 接受推荐方案 B：每个 inventory claim 使用不可变 typed evidence envelope，并明确标记 observed、inferred 或 unknown；默认摘要只返回 level、freshness、reason 与 evidence ID，详情按 ID 获取。
- observed 只来自当前 snapshot 内、声明边界中的直接 adapter read；inferred 只来自当前有效输入 evidence 和命名、版本化、确定性规则。路径惯例、相似度、概率、LLM/人工判断和未登记 fallback 不构成 inference，也不引入主观 confidence score。
- envelope 至少保存 subject/claim、method、collector/adapter/schema/rule/policy versions、snapshot、采集/求值时间与安全 provenance。unknown 是带稳定 reason 和 attempt evidence 的一等结果，不等于 absent/false/default/external-owned。
- freshness 按 claim 定义：不可变 revision-bound 事实以输入 revision/规则不变为有效条件；权限、登录、consumer version、active root 和 live 等可变事实要求 current snapshot 或 adapter/policy 最大年龄。没有全局 TTL，也不比较 Mac/Windows 时钟。
- 旧 envelope 不修改；过期、冲突或版本不兼容时，当前 claim 返回 unknown 并引用 stale/conflicting evidence。不得 latest-known、择新、择高版本或手工改 label。DEC-09/10 后续为动作声明最低 evidence 门槛；unknown 只阻断依赖它的写动作，安全 inventory/explain 可以继续。
- A/C/D 分别因丢失确定性 stage 推导、置信分数不可复算、以及陈旧证据继续驱动当前动作而拒绝。接受 adapter/schema 增加逐 claim method/freshness 合同与更多 unknown/block 的成本。

## DEC-07D 拍板结果

- 2026-07-17，principal 选择 B：每次 inventory snapshot 先固定 current host、declared targets、adapter/capability manifest、boundary revision 和 coverage units；每个 declared scope 分别记录 complete、partial、unavailable 或 not-applicable。
- coverage 与 07C evidence 正交：complete 只证明 scope 枚举完整，不提升单条 claim；partial/unavailable 不抹掉已取得 evidence，但未覆盖区域不得产生 absence、唯一 winner 或 collision-free 结论。observed absence 需要相关 coverage units 全部 complete。
- not-applicable 必须有 target capability/eligibility/policy 的正向证据，不能隐藏 adapter、权限或 boundary 缺口。已知 root 直接确认不存在可以是 complete + observed absence；root 未知、权限拒绝或遍历未完成只能是 partial/unavailable + unknown。
- 每次调用只盘点当前 host。另一台机器和 undeclared root/domain 是 out-of-scope，不是 unavailable；Windows snapshot 不包含也不以 not-applicable 暴露 Mac-local work。Almagest 不生成跨机 coverage 或中央汇总。
- partial/unavailable 只阻断依赖缺口的写动作，独立只读工作继续。修复由 principal 选择：外部权限交 owner/tool，受管 boundary/adapter 改动形成新 revision/plan；随后生成新 snapshot。Almagest 不自动扩扫、提权、读取 fallback/cache 或访问另一台 host。
- A/C/D 分别因过度丢弃有效局部证据、silent omission 制造假 absence、以及突破安全/驻留边界而拒绝。接受 adapter 声明 scope/complete condition、partial 缺口引用和更多局部 block 的成本。至此 DEC-07 四轴全部完成。
...
| 05C 决策记录 | add/duplicate/override/mask/remove/conflict 与硬策略边界均可审计 | 通过：05C 标记 `已拍板`；A/B/C/D、六种语义、Must/Later/Out、接受代价、后果和验收均已写入；表达模型与 target binding 均由 05D 固定 |
...
| 05D 决策记录 | source inventory、原生 payload、frontmatter/token、两源隔离与精确 target digest 均可审计 | 通过：05D 标记 `已拍板`；两组 A/B/C/D、Inventory/Payload/Target 合同、Must/Later/Out、接受代价、后果和验收均已写入 |
| 05D 结构完整性 | 完成 source representation 与 target binding，不新增 layer、不偷跑 DEC-06 binding 或 DEC-08 render | 通过：决策卡 16、独立决策轴 63、05D 决策节 1、验收场景 1、当前 DEC-05 状态 1；`git diff --check` 通过 |
| 05D 仓级回归 | 完成 DEC-05 全部四轴后不破坏现有工程 | 通过：隔离 `XDG_CONFIG_HOME` 后 ruff、format、pyrefly 通过，pytest 31/31 通过 |
| 06A 决策记录 | declaration、binding、observation 三分类、同名不同阶段、authority/layer 边界与未知分类均可审计 | 通过：06A 标记 `已拍板`；A/B/C/D、三类合同、Must/Later/Out、接受代价、后果和验收均已写入；06B—06D 保持未决 |
| 06A 结构完整性 | 只固定 data role，不偷跑 binding provider/scope、脱敏或缺值恢复 | 通过：决策卡 16、独立决策轴 63、06A 决策节 1、验收场景 1、当前 DEC-06 状态 1；`git diff --check` 通过 |
| 06A 仓级回归 | 固化 local-sensitive 三分类后不破坏现有工程 | 通过：隔离 `XDG_CONFIG_HOME` 后 ruff、format、pyrefly 通过，pytest 31/31 通过 |
| 06B 决策记录 | registry/provider/scope/match/replace/revision 与 host-local authority 边界均可审计 | 通过：06B 标记 `已拍板`；A/B/C/D、Registry/Provider/Scope 合同、Must/Later/Out、接受代价、后果和验收均已写入；06C—06D 保持未决 |
| 06B 结构完整性 | 固定 binding 受管入口但不偷跑 locator/value 脱敏或缺值恢复 | 通过：决策卡 16、独立决策轴 63、06B 决策节 1、验收场景 1、当前 DEC-06 状态 1；`git diff --check` 通过 |
| 06B 仓级回归 | 固化 typed host-local registry 后不破坏现有工程 | 通过：隔离 `XDG_CONFIG_HOME` 后 ruff、format、pyrefly 通过，pytest 31/31 通过 |
| 06C 决策记录 | sensitivity classes、per-surface allowlist、secret/local/unknown、reveal 与 provider error 边界均可审计 | 通过：06C 标记 `已拍板`；A/B/C/D、surface matrix、Must/Later/Out、接受代价、后果和验收均已写入；06D 保持未决 |
| 06C 结构完整性 | 固定安全输出但不偷跑 consumer secret render 或缺值恢复 | 通过：决策卡 16、独立决策轴 63、06C 决策节 1、验收场景 1、当前 DEC-06 状态 1；`git diff --check` 通过 |
| 06C 仓级回归 | 固化 schema-driven safe view 后不破坏现有工程 | 通过：隔离 `XDG_CONFIG_HOME` 后 ruff、format、pyrefly 通过，pytest 31/31 通过 |
| 06D 决策记录 | failure taxonomy、required/optional、阻断粒度、safe diagnostic、resolution authority 与重新验证均可审计 | 通过：06D 标记 `已拍板`；A/B/C/D、diagnostic matrix、Must/Later/Out、接受代价、后果和验收均已写入；DEC-06 四轴完成 |
| 06D 结构完整性 | 固定失败恢复合同但不偷跑多 target partial apply 或 plan schema 编码 | 通过：决策卡 16、独立决策轴 63、06D 决策节 1、验收场景 1、当前 DEC-06 状态 1；`git diff --check` 通过 |
| 06D 仓级回归 | 固化类型化 fail-closed 与现场决策后不破坏现有工程 | 通过：隔离 `XDG_CONFIG_HOME` 后 ruff、format、pyrefly 通过，pytest 31/31 通过 |
| 07A 决策记录 | stage/facts 范围、logical 粒度、lineage、bounded live、effective 隔离与敏感边界均可审计 | 通过：07A 标记 `已拍板`；A/B/C/D、stage matrix、Must/Later/Out、接受代价、后果和验收均已写入；07B—07D 保持未决 |
| 07A 结构完整性 | 固定 v1 inventory 粒度但不偷跑 ownership classification、evidence freshness 或 partial discovery | 通过：决策卡 16、独立决策轴 63、07A 决策节 1、验收场景 1、当前 DEC-07 状态 1；`git diff --check` 通过 |
| 07A 仓级回归 | 固化 logical stage inventory 后不破坏现有工程 | 通过：隔离 `XDG_CONFIG_HOME` 后 ruff、format、pyrefly 通过，pytest 31/31 通过 |
| 07B 决策记录 | control/integrity、external proof、unknown default、absence/orphan、duplicate 与 adoption 边界均可审计 | 通过：07B 标记 `已拍板`；A/B/C/D、classification matrix、Must/Later/Out、接受代价、后果和验收均已写入；07C—07D 保持未决 |
| 07B 结构完整性 | 固定 classification 但不偷跑 evidence confidence/freshness、root winner/shadow 或 lifecycle 实现 | 通过：决策卡 16、独立决策轴 63、07B 决策节 1、验收场景 1、当前 DEC-07 状态 1；active 状态值仅保留 `external-owned`，`unmanaged` 只出现在术语迁移或拒绝说明；`git diff --check` 通过 |
| 07B 仓级回归 | 固化正交 control/integrity 分类后不破坏现有工程 | 通过：隔离 `XDG_CONFIG_HOME` 后 ruff、format、pyrefly 通过，pytest 31/31 通过 |
| 07C 决策记录 | evidence level、envelope metadata、确定性 inference、unknown、freshness、失效与使用门禁均可审计 | 通过：07C 标记 `已拍板`；A/B/C/D、level matrix、Must/Later/Out、接受代价、后果和验收均已写入；07D 保持未决 |
| 07C 结构完整性 | 固定单条 claim 的证据语义但不偷跑 07D coverage、09/10 动作门槛或 11 runtime method | 通过：决策卡 16、独立决策轴 63、07C 决策节 1、验收场景 1、当前 DEC-07 状态 1；`git diff --check` 通过 |
| 07C 仓级回归 | 固化类型化证据信封与 claim-specific freshness 后不破坏现有工程 | 通过：隔离 `XDG_CONFIG_HOME` 后 ruff、format、pyrefly 通过，pytest 31/31 通过 |
| 07D 决策记录 | boundary、coverage states、absence gate、本机边界、局部阻断与恢复出口均可审计 | 通过：07D 标记 `已拍板`；A/B/C/D、coverage matrix、Must/Later/Out、接受代价、后果和验收均已写入；DEC-07 四轴完成 |
| 07D 结构完整性 | 固定 scope coverage 但不重定义 07C evidence、不偷跑 08A root precedence 或 09/10 action schema | 通过：决策卡 16、独立决策轴 63、07D 决策节 1、验收场景 1、当前 DEC-07 状态 1；`git diff --check` 通过 |
| 07D 仓级回归 | 固化 per-scope coverage map 与本机 partial snapshot 后不破坏现有工程 | 通过：隔离 `XDG_CONFIG_HOME` 后 ruff、format、pyrefly 通过，pytest 31/31 通过 |
...
- DEC-08—DEC-16 的拍板结果：后续逐项写入 `design.md`。
