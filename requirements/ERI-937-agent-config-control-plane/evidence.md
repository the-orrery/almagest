...
- live discovery 只进入 adapter/target 声明的 config root、file type、config key 与引用边界；边界内 unmapped candidate 可保留 opaque evidence，但不自动取得 logical/managed/source identity 或 adopt/delete 权限。07B/07C 已固定 classification 与 evidence level/freshness；07D 继续定义 partial discovery。
...
- DEC-07D—DEC-16 的拍板结果：后续逐项写入 `design.md`。
