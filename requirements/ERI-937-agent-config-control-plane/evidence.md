...
- override/mask 只允许 Mac-local work 无歧义地指向 eligible base 的 semantic target。target 缺失/歧义、shape 不兼容或多个意图竞争均 conflict/block；target reference 的字段、是否绑定 expected revision/digest、序列化与目录布局留给 05D。
...
- A/C/D 分别因静默 last-layer-wins、无法表达真实 Mac-only 覆盖、以及把稳定 overlay 变成每次重复的 transient exception 而拒绝。接受每项显式意图和 base 更新后 stale 告警的成本。
...
| 05C 决策记录 | add/duplicate/override/mask/remove/conflict 与硬策略边界均可审计 | 通过：05C 标记 `已拍板`；A/B/C/D、六种语义、Must/Later/Out、接受代价、后果和验收均已写入；target reference 与表达语法保持未决 |
...
- DEC-05D 与 DEC-06—DEC-16 的拍板结果：后续逐项写入 `design.md`。
