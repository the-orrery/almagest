...
- consumer 配置中的 registry/ref/version constraint 仍可作为普通 desired data 被投影，但不会触发 package 拉取，也不会让外部内容取得 source authority。吸收后的内容重新经过正常配置闭环与 03D 风险策略；owned 不等于安全。
...
该体检保留 03A 的固定 source-class ownership、03B1/03B2、01A 的配置域和 01C 的不可变 revision 语义；只重画外部 acquisition plane 边界。03D 仍未拍板，不能从“已吸收/已 owned”推导为安全或已批准。
...
- DEC-03D 与 DEC-04—DEC-16 的拍板结果：后续逐项写入 `design.md`。
