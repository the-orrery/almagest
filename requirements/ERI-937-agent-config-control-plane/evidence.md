...
- live discovery 只进入 adapter/target 声明的 config root、file type、config key 与引用边界；边界内 unmapped candidate 可保留 opaque evidence，但不自动取得 logical/managed/source identity 或 adopt/delete 权限。07B—07D 继续定义 classification、evidence quality/freshness 与 partial discovery。
...
- managed 必须有当前合法 source/registry 或固定 plan/apply/receipt ownership evidence；external-owned 必须有正向 external owner identity/evidence；缺少 managed provenance 不能反推出 external-owned，证据不足、冲突或失效均为 unknown-owner。07C 继续固定证据等级与 freshness。
...
- DEC-07C—DEC-16 的拍板结果：后续逐项写入 `design.md`。
