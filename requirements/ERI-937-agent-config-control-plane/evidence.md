...
- 恢复动作完成后必须重新 inventory/verify，只有证据证明越界 payload 已消失且输入重新固定才解除阻断。检测调度、通知渠道、脱敏跨机摘要和具体 schema 留给 04D、DEC-07/09/12/13/16。
...
- DEC-04D 与 DEC-05—DEC-16 的拍板结果：后续逐项写入 `design.md`。
