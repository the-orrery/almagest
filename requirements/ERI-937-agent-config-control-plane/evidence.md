...
- 临时 apply 必须保持 exception 状态并可审计，不冒充稳定 `compliant`。B 因形成第二份配置真相被拒；C 因默认扩大为 source 修改被拒；D 因路径与状态面过多被拒。03B2 的派生物反向污染检测仍待拍板。
...
- DEC-03B2—03D 与 DEC-04—DEC-16 的拍板结果：后续逐项写入 `design.md`。
