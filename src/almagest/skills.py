...
def apply(m: Manifest, *, force: bool = False) -> list[LinkOp]:
    ...
    changed: list[LinkOp] = []
    for op in plan(m):
        if op.status in {"missing-src", "unknown-lane", "ok"}:
            continue
...
def composition_snapshots(
    ...
    *,
    ...
    expected 按目标目录取所有 lane 的并集,即 base ∪ overlay;actual 只看目录入口名,
...
def composition_problems(
    ...
    *,
    ...
    """断言每个 runtime 目录实际 skill 集等于 base ∪ overlays。"""
    allowed = allow_extra or set()
    problems: list[ActiveOp] = []
