...
from almagest import __version__
...
_ENV_DB = "ALMAGEST_TELEMETRY_DB"
_ENV_OFF = "ALMAGEST_TELEMETRY_OFF"


def db_path() -> Path:
    """Resolve the ledger path: $ALMAGEST_TELEMETRY_DB override,
    else ($XDG_DATA_HOME or $HOME/.local/share)/almagest/telemetry.db."""
    override = os.environ.get(_ENV_DB)
    if override:
        return Path(override)
    base = os.environ.get("XDG_DATA_HOME") or str(Path.home() / ".local" / "share")
    return Path(base) / "almagest" / "telemetry.db"
...
def stats(path: Path | None = None) -> str:
    ...
    lines = [
        f"almagest telemetry — {total} calls · {total_err} errors ({rate:.1f}%)",
        "",
    ]
